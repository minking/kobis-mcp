...
| 도구명 | 설명 | 주요 파라미터 |
|---|---|---|
| **`get_daily_boxoffice`** | 특정 일자 일별 박스오피스 TOP 10 및 관객수/매출액 조회 | `targetDt` (YYYYMMDD), `multiMovieYn`, `repNationCd`, `wideAreaCd` |
| **`get_weekly_boxoffice`** | 주말(금~일) 또는 주간(월~일) 박스오피스 순위 및 관객수 조회 | `targetDt` (일요일 YYYYMMDD), `weekGb` (0:주간, 1:주말, 2:주중) |
